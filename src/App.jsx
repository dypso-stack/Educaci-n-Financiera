...
const LESSON_EXTRAS = {
  1: {
    time: "4–5 min",
    objective: "Entender cómo ordenar ingresos y gastos para tomar decisiones con más control.",
    why: "Un presupuesto no limita: te ayuda a anticiparte, evitar desorden y saber cuánto sí puedes gastar.",
    example: "Si ganas $1,000 y no registras tus gastos, es fácil sentir que ‘no te alcanza’ sin saber por qué.",
    mistake: "Confiarte en la memoria o ahorrar solo si sobra dinero.",
    takeaway: "Primero organiza, luego decide. Lo que no se mide, se desordena."
  },
  2: {
    time: "3–4 min",
    objective: "Reconocer detonantes del gasto impulsivo y aplicar frenos prácticos.",
    why: "Muchas malas decisiones no vienen por falta de ingresos, sino por compras rápidas y poco pensadas.",
    example: "Una compra en cuotas puede parecer pequeña al mes, pero terminar costando mucho más de lo que imaginabas.",
    mistake: "Pensar solo en la cuota y no en el costo total.",
    takeaway: "No todo lo pagable es conveniente."
  },
  3: {
    time: "4–5 min",
    objective: "Diferenciar cuándo conviene ahorrar y cuándo conviene invertir.",
    why: "Confundir ahorro con inversión puede dejarte sin liquidez para emergencias o con dinero quieto perdiendo valor.",
    example: "Un fondo para una emergencia médica no debería estar en un instrumento riesgoso.",
    mistake: "Invertir sin antes tener un colchón básico.",
    takeaway: "Primero liquidez y seguridad; luego crecimiento."
  },
  4: {
    time: "3–4 min",
    objective: "Identificar señales comunes de fraude y reaccionar rápido si ocurre un incidente.",
    why: "Hoy los fraudes digitales usan urgencia, miedo y suplantación para presionar decisiones inmediatas.",
    example: "Un mensaje que pide ‘confirmar tu cuenta ahora’ suele buscar que actúes sin verificar.",
    mistake: "Hacer clic antes de confirmar por un canal oficial.",
    takeaway: "Ante presión o urgencia, pausa y verifica."
  }
};
...
export default function FinanzasTaller() {
  ...
  const openLesson = (l) => { setActiveLesson(l); setLessonStep(0); setScreen("lesson"); };
  ...
  const renderBlock = (lesson, item, i) => {
    if (item.type === "overview") return (
      <div key={i} style={{ ...blkWrap, padding: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <div style={{ fontSize: 18, fontWeight: 900, color: lesson.color }}>{lesson.icon} {item.title}</div>
          <div style={{ fontSize: 11, fontWeight: 800, color: lesson.color, background: `${lesson.color}12`, border: `1px solid ${lesson.color}30`, padding: "5px 10px", borderRadius: 999 }}>{item.time}</div>
        </div>
        <div style={{ fontSize: 13, color: C.textMid, lineHeight: 1.65, marginBottom: 14 }}>{lesson.summary}</div>
        <div style={{ display: "grid", gap: 10 }}>
          <div style={{ background: C.bgSoft, border: `1px solid ${C.border}`, borderRadius: 12, padding: 12 }}><div style={{ fontSize: 11, fontWeight: 800, color: lesson.color, letterSpacing: 1, textTransform: "uppercase", marginBottom: 6 }}>Objetivo</div><div style={{ fontSize: 13, color: C.textMid, lineHeight: 1.55 }}>{item.objective}</div></div>
          <div style={{ background: C.bgSoft, border: `1px solid ${C.border}`, borderRadius: 12, padding: 12 }}><div style={{ fontSize: 11, fontWeight: 800, color: lesson.color, letterSpacing: 1, textTransform: "uppercase", marginBottom: 6 }}>Por qué importa</div><div style={{ fontSize: 13, color: C.textMid, lineHeight: 1.55 }}>{item.why}</div></div>
          <div style={{ background: `${lesson.color}08`, border: `1px solid ${lesson.color}22`, borderRadius: 12, padding: 12 }}><div style={{ fontSize: 11, fontWeight: 800, color: lesson.color, letterSpacing: 1, textTransform: "uppercase", marginBottom: 6 }}>Ejemplo rápido</div><div style={{ fontSize: 13, color: C.textMid, lineHeight: 1.55 }}>{item.example}</div></div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div style={{ background: C.warnBg, border: `1px solid #F59E0B`, borderRadius: 12, padding: 12 }}><div style={{ fontSize: 11, fontWeight: 800, color: "#92400E", letterSpacing: 1, textTransform: "uppercase", marginBottom: 6 }}>Error común</div><div style={{ fontSize: 12, color: C.warnText, lineHeight: 1.55 }}>{item.mistake}</div></div>
            <div style={{ background: `${C.tertiary}28`, border: `1px solid ${C.tertiary}`, borderRadius: 12, padding: 12 }}><div style={{ fontSize: 11, fontWeight: 800, color: C.secondary, letterSpacing: 1, textTransform: "uppercase", marginBottom: 6 }}>Qué te llevas</div><div style={{ fontSize: 12, color: C.secondary, lineHeight: 1.55 }}>{item.takeaway}</div></div>
          </div>
        </div>
      </div>
    );
  ...
  if (screen === "home") return (
    <div style={appSt}>
      ...
      <div style={{ padding: "18px 18px 80px" }}>
        <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 2.5, color: C.textLight, textTransform: "uppercase", marginBottom: 16 }}>Elige una ruta</div>
        ...
        <div style={{ ...cardSt, flexDirection: "column", alignItems: "flex-start", gap: 10, borderLeft: `4px solid ${C.primary}` }} onClick={() => setScreen("learn")} onMouseEnter={e => hover(e, C.primary, true)} onMouseLeave={e => hover(e, C.primary, false)}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, width: "100%" }}>
            ...
            <div style={{ flex: 1 }}><div style={{ fontSize: 16, fontWeight: 900, color: C.primary }}>Aprender</div><div style={{ fontSize: 12, color: C.textSoft, marginTop: 2 }}>Tarjetas guiadas con contenido más detallado y ejemplos prácticos</div></div>
        ...
        <div style={{ ...cardSt, flexDirection: "column", alignItems: "flex-start", gap: 10, borderLeft: `4px solid ${C.secondary}` }} onClick={() => setScreen("quizmode")} onMouseEnter={e => hover(e, C.secondary, true)} onMouseLeave={e => hover(e, C.secondary, false)}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, width: "100%" }}>
            ...
            <div style={{ flex: 1 }}><div style={{ fontSize: 16, fontWeight: 900, color: C.secondary }}>Poner a prueba mis conocimientos</div><div style={{ fontSize: 12, color: C.textSoft, marginTop: 2 }}>Quiz ágil para validar conceptos clave y reforzar aprendizaje</div></div>
        ...
        <div style={{ ...cardSt, flexDirection: "column", alignItems: "flex-start", gap: 10, borderLeft: `4px solid ${C.quat}` }} onClick={() => { setGameScreen("intro"); setScreen("game"); }} onMouseEnter={e => hover(e, C.quat, true)} onMouseLeave={e => hover(e, C.quat, false)}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, width: "100%" }}>
            ...
            <div style={{ flex: 1 }}><div style={{ fontSize: 16, fontWeight: 900, color: C.quat }}>Juego interactivo</div><div style={{ fontSize: 12, color: C.textSoft, marginTop: 2 }}>Experiencia breve, dinámica y enfocada en decisiones</div></div>
            <span style={{ color: C.quat, fontSize: 18 }}>→</span>
          </div>
          <div style={{ fontSize: 12, color: C.textSoft, padding: "7px 11px", background: `${C.quat}08`, border: `1px solid ${C.quat}30`, borderRadius: 10 }}>Modo corto y gamificado para aplicar lo aprendido sin leer demasiado.</div>
        </div>
      </div>
    </div>
  );
  ...
  if (screen === "learn") return (
    <div style={appSt}>
      ...
      <div style={{ padding: "20px 18px 80px" }}>
        ...
        <div style={{ fontSize: 13, color: C.textSoft, marginBottom: 20 }}>Tarjetas con más detalle, ejemplos y errores comunes para reforzar lo básico</div>
        {LESSONS.map(l => (
          <div key={l.id} style={{ ...cardSt, borderLeft: `4px solid ${l.color}` }} onClick={() => openLesson(l)} onMouseEnter={e => hover(e, l.color, true)} onMouseLeave={e => hover(e, l.color, false)}>
            ...
            <div style={{ flex: 1 }}><div style={{ fontSize: 15, fontWeight: 800, color: l.color, marginBottom: 3 }}>{l.title}</div><div style={{ fontSize: 12, color: C.textSoft, lineHeight: 1.4, marginBottom: 8 }}>{l.summary}</div><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}><span style={{ fontSize: 10, padding: "4px 8px", borderRadius: 999, background: `${l.color}12`, color: l.color, fontWeight: 700 }}>{LESSON_EXTRAS[l.id].time}</span><span style={{ fontSize: 10, padding: "4px 8px", borderRadius: 999, background: C.bgSoft, color: C.textSoft, border: `1px solid ${C.border}` }}>Tarjetas guiadas</span></div></div>
  ...
  if (screen === "lesson" && activeLesson) {
    ...
    const lessonCards = buildLessonCards(l);
    const currentCard = lessonCards[lessonStep];
    const lessonProgress = Math.round(((lessonStep + 1) / lessonCards.length) * 100);
    const isLastCard = lessonStep === lessonCards.length - 1;
    return (
      <div style={appSt}>
        <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;600;700;800;900&display=swap" rel="stylesheet" />
        ...
        <div style={{ padding: "16px 18px 60px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <div style={{ fontSize: 12, color: C.textSoft }}>Tarjeta {lessonStep + 1} de {lessonCards.length}</div>
            <div style={{ fontSize: 12, fontWeight: 800, color: l.color }}>{lessonProgress}%</div>
          </div>
          <div style={{ background: C.border, borderRadius: 999, height: 7, overflow: "hidden", marginBottom: 16 }}>
            <div style={{ height: "100%", width: `${lessonProgress}%`, background: `linear-gradient(90deg, ${l.color}, ${l.accent})`, borderRadius: 999, transition: "width 0.3s ease" }} />
          </div>
          {renderBlock(l, currentCard, lessonStep)}
          <div style={{ display: "flex", gap: 10, margin: "8px 18px 0" }}>
            <button onClick={() => setLessonStep(s => Math.max(0, s - 1))} disabled={lessonStep === 0} style={{ flex: 1, padding: 14, borderRadius: 14, background: lessonStep === 0 ? C.bgSoft : C.bg, border: `1px solid ${C.border}`, color: lessonStep === 0 ? C.textLight : C.textMid, fontSize: 14, fontWeight: 800, cursor: lessonStep === 0 ? "default" : "pointer", fontFamily: "inherit" }}>← Anterior</button>
            {!isLastCard ? (
              <button onClick={() => setLessonStep(s => Math.min(lessonCards.length - 1, s + 1))} style={{ flex: 1.2, padding: 14, borderRadius: 14, background: `linear-gradient(135deg, ${l.color}, ${l.accent})`, border: "none", color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}>Siguiente →</button>
            ) : (
              <button onClick={() => { setCompleted(c => new Set([...c, l.id])); setXp(x => x + 25); setScreen("learn"); }} style={{ flex: 1.2, padding: 14, borderRadius: 14, background: `linear-gradient(135deg, ${l.color}, ${l.accent})`, border: "none", color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}>✓ Completar lección</button>
            )}
          </div>
        </div>
      </div>
      ...
      <div style={appSt}>
        ...
        {gameScreen === "intro" && (
          <div style={{ padding: "32px 18px" }}>
            ...
            <div style={{ fontSize: 14, color: C.textMid, lineHeight: 1.65, marginBottom: 24 }}>Escenarios cortos, directos y orientados a aplicar lo aprendido.</div>
