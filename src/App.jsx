...
export default function FinanzasTaller() {
  ...
  const nextScenario = () => {
    ...
    if (gameIndex + 1 >= activeScenarios.length) {
      const finalScore = gameHistory.reduce((sum, h) => sum + h.points, 0);
  ...
  if (screen === "home") return (
    <div style={appSt}>
      ...
      <div style={{ padding: "18px 18px 80px" }}>
        ...
        <div style={{ ...cardSt, flexDirection: "column", alignItems: "flex-start", gap: 10, borderLeft: `4px solid ${C.quat}` }} onClick={() => { resetGame(); setScreen("game"); }} onMouseEnter={e => hover(e, C.quat, true)} onMouseLeave={e => hover(e, C.quat, false)}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, width: "100%" }}>
  ...
  // ══ QUIZ MODE ══════════════════════════════
  if (screen === "quizmode") return <QuizMode xp={xp} setXp={setXp} onBack={() => setScreen("home")} onGoGame={() => { resetGame(); setScreen("game"); }} onGoLearn={() => setScreen("learn")} onComplete={() => setCompletedModules(m => ({ ...m, quiz: true }))} />;
