...
const RESPONSIVE_CSS = `
  * { box-sizing: border-box; }
  ...
  @media (min-width: 900px) {
    body { background: linear-gradient(135deg, #160F41 0%, #2a1f6e 45%, #A31A61 100%); min-height: 100vh; }
    #root { display: flex; justify-content: center; align-items: flex-start; gap: 24px; padding: 32px 24px; }
    .app-main { width: 500px; flex-shrink: 0; border-radius: 24px; overflow: hidden; box-shadow: 0 24px 80px rgba(0,0,0,.4); }
    .app-sidebar { width: 260px; flex-shrink: 0; display: flex; flex-direction: column; gap: 14px; position: sticky; top: 32px; }
  }
...
function Sidebar({ screen, activeLesson, completed, xp, allLessonsCompleted, completedModules, onNav }) {
  ...
  return (
    <>
      <div style={{ background: `linear-gradient(160deg, ${C.secondary}, #2a1f6e)`, borderRadius: 18, padding: "18px 16px", color: "#fff" }}>
        <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 1.4, textTransform: "uppercase", opacity: 0.7, marginBottom: 10 }}>Tu progreso</div>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          <span style={{ fontSize: 28 }}>{lvl.icon}</span>
          <div>
            <div style={{ fontSize: 15, fontWeight: 900 }}>{lvl.label}</div>
            <div style={{ fontSize: 12, opacity: 0.8 }}>Nv. {lvl.level} · {xp} XP</div>
          </div>
        </div>
        {nextLvl && (<>
          <div style={{ background: "rgba(255,255,255,.2)", borderRadius: 999, height: 5, overflow: "hidden", marginBottom: 5 }}>
            <div style={{ height: "100%", width: `${pct}%`, borderRadius: 999, background: `linear-gradient(90deg, ${C.highlight}, #fff)`, transition: "width .5s" }} />
          </div>
          <div style={{ fontSize: 11, opacity: 0.65, marginBottom: 14 }}>{nextLvl.min - xp} XP para {nextLvl.label}</div>
        </>)}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          {[{ label: "Lecciones", value: `${completed.size}/${LESSONS.length}` }, { label: "Ruta", value: `${overallPct}%` }].map((s, i) => (
            <div key={i} style={{ background: "rgba(255,255,255,.12)", borderRadius: 10, padding: "8px 10px" }}>
              <div style={{ fontSize: 10, opacity: 0.65, marginBottom: 2 }}>{s.label}</div>
              <div style={{ fontSize: 16, fontWeight: 900 }}>{s.value}</div>
            </div>
          ))}
        </div>
      </div>
      <div style={{ background: "rgba(255,255,255,.95)", borderRadius: 18, padding: "14px 12px" }}>
        <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 1.4, textTransform: "uppercase", color: C.textLight, marginBottom: 10 }}>Navegación</div>
        {navItems.map((item) => {
          ...
          return (
            <button key={item.target} onClick={() => !item.locked && onNav(item.target)}
              style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "9px 10px", borderRadius: 10,
                background: active ? `${C.primary}12` : "transparent", border: active ? `1px solid ${C.primary}22` : "1px solid transparent",
                color: item.locked ? C.textLight : active ? C.primary : C.textMid,
                fontSize: 13, fontWeight: active ? 800 : 600, cursor: item.locked ? "not-allowed" : "pointer",
                fontFamily: "inherit", textAlign: "left", marginBottom: 2, opacity: item.locked ? 0.5 : 1 }}>
              <span style={{ fontSize: 15 }}>{item.icon}</span>
              <span style={{ flex: 1 }}>{item.label}</span>
              {item.locked && <span style={{ fontSize: 11 }}>🔒</span>}
              {active && <span style={{ width: 6, height: 6, borderRadius: "50%", background: C.primary, display: "inline-block" }} />}
            </button>
          );
        })}
      </div>
      <div style={{ background: "rgba(255,255,255,.95)", borderRadius: 18, padding: "14px 16px" }}>
        <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 1.4, textTransform: "uppercase", color: C.textLight, marginBottom: 10 }}>
          {activeLesson ? `💡 ${activeLesson.title}` : "💡 Tips financieros"}
        </div>
        {tips.map((tip, i) => (
          <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginBottom: i < tips.length - 1 ? 10 : 0 }}>
            <div style={{ minWidth: 6, height: 6, borderRadius: "50%", background: tipColor, marginTop: 6, flexShrink: 0 }} />
            <span style={{ fontSize: 12, color: C.textMid, lineHeight: 1.6 }}>{tip}</span>
          </div>
        ))}
      </div>
    </>
...
function QuizMode({ xp, setXp, onBack, onGoGame, onGoLearn, onGoHome, onComplete, rewardEnabled = true }) {
  ...
  const appSt = { fontFamily: "'Plus Jakarta Sans',sans-serif", background: C.bgSoft, minHeight: "100vh", color: C.text, maxWidth: 500, margin: "0 auto", boxShadow: "0 0 60px rgba(22,15,65,.10)" };
