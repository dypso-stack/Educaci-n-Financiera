...
const STORAGE_KEY = "bg_finanzas_taller_state_v2";
...
const LESSONS = [
  ...
  },
  {
    ...
    color: C.secondary, accent: "#0E5A68",
...
const GAME_SCENARIO_BANK = [
  ...
  { situation: "Tu banco te ofrece aumentar tu límite de tarjeta de $2,000 a $5,000 sin costo. ¿Lo aceptas?", options: [
    ...
    { text: "No, un límite alto me tentará a gastar más", points: 10, feedback: "¡Excelente autoconocimiento! Reconocer que un límite mayor puede distorsionar tus hábitos es una señal de madurez financiera. Es mejor un límite que te obligue a ser disciplinado." },
  ]},
    ...
    >
      ...
      <div
        style={{
          ...
          boxShadow: "0 6px 18px rgba(163,26,97,.08)",
        }}
      >
        <span style={{ fontSize: 14 }}>⚡</span>
        <div>
          <div style={{ fontSize: 10, fontWeight: 800, color: C.textLight, lineHeight: 1 }}>XP</div>
          <div style={{ fontSize: 13, fontWeight: 900, color: C.primary, lineHeight: 1.1 }}>{xp}</div>
        </div>
      </div>
    </div>
  );
...
function QuizMode({ xp, setXp, onBack, onGoGame, onGoLearn, onComplete, rewardEnabled = true }) {
  ...
  if (done) return (
    <div style={appSt}>
      ...
      <div style={{ padding: "30px 18px 40px" }}>
        <div style={{ background: `linear-gradient(160deg, ${C.secondary} 0%, #21135A 58%, ${C.primary} 100%)`, borderRadius: 26, padding: "28px 20px", textAlign: "center", color: "#fff", boxShadow: "0 18px 40px rgba(22,15,65,.18)", marginBottom: 18 }}>
      ...
      <div style={{ padding: "18px 18px 80px" }}>
        ...
        <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 22, padding: 18, marginBottom: 18, boxShadow: "0 10px 26px rgba(22,15,65,.06)" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: C.textSoft }}>
            ...
            <span>{rewardEnabled ? "Hasta +500 XP" : "Modo práctica"}</span>
          </div>
        </div>
          ...
          {answer !== null && (
            <>
              <div style={{ padding: "13px 14px", borderRadius: 16, background: `${C.tertiary}28`, border: `1.5px solid ${C.tertiary}`, fontSize: 13, color: C.secondary, lineHeight: 1.6, marginBottom: 14 }}>
                {answer === q.a ? "✅ ¡Correcto! " : "❌ Incorrecto. "}{q.exp}{answer === q.a && rewardEnabled && <span style={{ color: C.primary, fontWeight: 800 }}> +50 XP</span>}
              </div>
...
export default function FinanzasTaller() {
  ...
  }, [xp, completed, completedModules, soundOn]);
  ...
  const handleGameChoice = (oi) => {
    ...
    setGameHistory(h => [...h, { scenario: gameIndex, choice: oi, points: opt.points }]);
  ...
  const nextScenario = () => {
    ...
    if (gameIndex + 1 >= activeScenarios.length) {
      ...
      const award = finalScore >= 30 && !completedModules.game ? 40 : 0;
      if (award > 0) setXp(x => x + award);
  ...
  if (screen === "home") return (
    <div style={appSt}>
      ...
      <div style={{ padding: "18px 18px 88px" }}>
        <div style={{ background: `linear-gradient(160deg, ${C.secondary} 0%, #21135A 58%, ${C.primary} 100%)`, borderRadius: 28, padding: "22px 20px", color: "#fff", boxShadow: "0 18px 40px rgba(22,15,65,.18)", marginBottom: 18, position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 16 }}>
              <div style={{ background: "rgba(255,255,255,.12)", border: "1px solid rgba(255,255,255,.12)", borderRadius: 18, padding: "12px 10px" }}>
                <div style={{ fontSize: 10, fontWeight: 800, opacity: .78, letterSpacing: 1.1, textTransform: "uppercase" }}>Lecciones</div>
                <div style={{ fontSize: 22, fontWeight: 900, marginTop: 3 }}>4</div>
              </div>
              <div style={{ background: "rgba(255,255,255,.12)", border: "1px solid rgba(255,255,255,.12)", borderRadius: 18, padding: "12px 10px" }}>
                <div style={{ fontSize: 10, fontWeight: 800, opacity: .78, letterSpacing: 1.1, textTransform: "uppercase" }}>Quiz</div>
                <div style={{ fontSize: 22, fontWeight: 900, marginTop: 3 }}>10</div>
              </div>
              <div style={{ background: "rgba(255,255,255,.12)", border: "1px solid rgba(255,255,255,.12)", borderRadius: 18, padding: "12px 10px" }}>
                <div style={{ fontSize: 10, fontWeight: 800, opacity: .78, letterSpacing: 1.1, textTransform: "uppercase" }}>Juego</div>
                <div style={{ fontSize: 22, fontWeight: 900, marginTop: 3 }}>4</div>
              </div>
            </div>
            ...
            <div style={{ background: "rgba(255,255,255,.18)", borderRadius: 999, height: 10, overflow: "hidden" }}>
              <div style={{ height: "100%", borderRadius: 999, background: "linear-gradient(90deg, #fff, #F1C4DC)", width: `${progress}%`, transition: "width 0.7s ease" }} />
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: 1.4, color: C.textLight, textTransform: "uppercase", marginBottom: 4 }}>Ruta disponible</div>
            <div style={{ fontSize: 18, fontWeight: 900, color: C.secondary }}>Escoge tu siguiente paso</div>
          </div>
        ...
        <div style={{ ...cardSt, flexDirection: "column", alignItems: "flex-start", gap: 12, border: `1px solid ${C.primary}22`, boxShadow: "0 12px 28px rgba(22,15,65,.06)" }} onClick={() => setScreen("learn")} onMouseEnter={e => hover(e, C.primary, true)} onMouseLeave={e => hover(e, C.primary, false)}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 14, width: "100%" }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 18, fontWeight: 900, color: C.primary, marginBottom: 4 }}>Aprender</div>
              <div style={{ fontSize: 13, color: C.textSoft, lineHeight: 1.55 }}>Explora los fundamentos de presupuesto, hábitos, ahorro, inversión y prevención de fraudes.</div>
            </div>
        ...
        <div style={{ ...cardSt, flexDirection: "column", alignItems: "flex-start", gap: 12, border: `1px solid ${C.secondary}20`, boxShadow: "0 12px 28px rgba(22,15,65,.06)" }} onClick={() => setScreen("quizmode")} onMouseEnter={e => hover(e, C.secondary, true)} onMouseLeave={e => hover(e, C.secondary, false)}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, width: "100%" }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 18, fontWeight: 900, color: C.secondary, marginBottom: 4 }}>Pon a prueba tus conocimientos {completedModules.quiz ? "✅" : ""}</div>
              <div style={{ fontSize: 13, color: C.textSoft, lineHeight: 1.55 }}>10 preguntas aleatorias para reforzar lo aprendido y ganar experiencia.</div>
            </div>
            <div style={{ fontSize: 18, color: C.secondary }}>{completedModules.quiz ? "✓" : "→"}</div>
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 11, padding: "6px 10px", borderRadius: 999, background: `${C.tertiary}24`, color: C.secondary, border: `1px solid ${C.tertiary}`, fontWeight: 800 }}>Hasta +500 XP</span>
          </div>
        </div>

        <div style={{ ...cardSt, flexDirection: "column", alignItems: "flex-start", gap: 12, border: `1px solid ${C.quat}20`, boxShadow: "0 12px 28px rgba(22,15,65,.06)" }} onClick={() => { resetGame(); setScreen("game"); }} onMouseEnter={e => hover(e, C.quat, true)} onMouseLeave={e => hover(e, C.quat, false)}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, width: "100%" }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 18, fontWeight: 900, color: C.quat, marginBottom: 4 }}>Juego interactivo {completedModules.game ? "✅" : ""}</div>
              <div style={{ fontSize: 13, color: C.textSoft, lineHeight: 1.55 }}>Toma decisiones en escenarios financieros reales y descubre el impacto de cada elección.</div>
            </div>
            <div style={{ fontSize: 18, color: C.quat }}>{completedModules.game ? "✓" : "→"}</div>
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 11, padding: "6px 10px", borderRadius: 999, background: `${C.quat}10`, color: C.quat, border: `1px solid ${C.quat}26`, fontWeight: 800 }}>Hasta +40 XP</span>
          </div>
        </div>
      </div>
    </div>
  ...
  if (screen === "learn") return (
    <div style={appSt}>
      ...
      <div style={{ padding: "18px 18px 80px" }}>
        ...
        {LESSONS.map((l, index) => (
          <div key={l.id} style={{ ...cardSt, border: `1px solid ${l.color}18`, borderRadius: 22, padding: 18, boxShadow: "0 10px 24px rgba(22,15,65,.05)" }} onClick={() => openLesson(l)} onMouseEnter={e => hover(e, l.color, true)} onMouseLeave={e => hover(e, l.color, false)}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                ...
                <span style={{ fontSize: 10, padding: "4px 8px", borderRadius: 999, background: completed.has(l.id) ? `${l.color}12` : C.bgSoft, border: `1px solid ${completed.has(l.id) ? l.color + "30" : C.border}`, color: completed.has(l.id) ? l.color : C.textSoft, fontWeight: 800 }}>
                  {completed.has(l.id) ? "Completada" : `Lección ${index + 1}`}
                </span>
              </div>
              <div style={{ fontSize: 13, color: C.textSoft, lineHeight: 1.55, marginBottom: 10 }}>{l.summary}</div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <span style={{ fontSize: 11, padding: "5px 8px", borderRadius: 999, background: C.bgSoft, color: C.textMid, border: `1px solid ${C.border}`, fontWeight: 700 }}>{l.content.length} bloques</span>
                <span style={{ fontSize: 11, padding: "5px 8px", borderRadius: 999, background: `${l.color}0F`, color: l.color, border: `1px solid ${l.color}20`, fontWeight: 700 }}>Acción práctica</span>
              </div>
            </div>
            <div style={{ fontSize: 18, color: l.color }}>{completed.has(l.id) ? "✅" : "→"}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  // ══ QUIZ MODE ══════════════════════════════
  if (screen === "quizmode") return <QuizMode xp={xp} setXp={setXp} rewardEnabled={!completedModules.quiz} onBack={() => setScreen("home")} onGoGame={() => { resetGame(); setScreen("game"); }} onGoLearn={() => setScreen("learn")} onComplete={() => setCompletedModules(m => ({ ...m, quiz: true }))} />;
    ...
    return (
      <div style={appSt}>
        ...
        <div style={{ padding: "38px 18px" }}>
          ...
          <button onClick={() => { resetGame(); setScreen("game"); }} style={{ width: "100%", padding: 15, borderRadius: 14, background: C.bgSoft, border: `1px solid ${C.border}`, color: C.textMid, fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}>
            Ir al juego 🎮
          </button>
        </div>
      </div>
  ...
  if (screen === "game") {
    ...
    const treasurePos = { left: "88%", top: "10%" };

    // Character sits at last completed waypoint (or start)
    const charStep = gameScreen === "result" ? total : gameIndex;
    const charPos = charStep === 0 ? { left: "12%", top: "82%" } : waypoints[Math.min(charStep - 1, total - 1)];

    // Result tier — máximo posible: 4 escenarios × 10 pts = 40 pts
    const tier = gameScore >= 30 ? "gold" : gameScore >= 10 ? "silver" : "bronze";
    const tierData = {
      gold:   { label: "¡Experto Financiero!", color: "#D97706", bg: "linear-gradient(135deg,#FDE68A,#F59E0B)", icon: "🏆", xpLabel: "+40 XP" },
      silver: { label: "Buen Progreso",         color: "#6B7280", bg: "linear-gradient(135deg,#E5E7EB,#9CA3AF)", icon: "🥈", xpLabel: "" },
      bronze: { label: "Sigue Practicando",     color: "#92400E", bg: "linear-gradient(135deg,#FDE68A,#D97706)", icon: "📚", xpLabel: "" },
    }[tier];
    ...
    const keyframes = `
      ...
      @keyframes glow     { 0%,100%{box-shadow:0 0 12px #D2006E55} 50%{box-shadow:0 0 28px #D2006Eaa} }
      ...
      <div style={appSt}>
        ...
        {gameScreen === "intro" && (
          <div style={{ padding: "0 0 32px" }}>
            <div style={{ background: `linear-gradient(160deg, ${C.secondary} 0%, #21135A 55%, ${C.primary} 100%)`, padding: "28px 20px 32px", position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ fontSize:13, color:"rgba(255,255,255,.78)", lineHeight:1.65, marginBottom:16 }}>
                Avanza por 4 decisiones reales, recibe retroalimentación inmediata y desbloquea tu tesoro final si tomas buenas decisiones.
              </div>

              <div style={{ display:"grid", gridTemplateColumns:"repeat(3, minmax(0,1fr))", gap:10 }}>
                {[
                  { label:"Paradas", value:"4" },
                  { label:"XP máximo", value:"40" },
            ...
            <div style={{ margin:"20px 18px 0", background: C.bg, border:`1px solid ${C.border}`, borderRadius:24, padding:"22px 18px", boxShadow:"0 10px 28px rgba(22,15,65,0.08)" }}>
              ...
              <div style={{ position:"relative", height:74, marginBottom:18 }}>
                <svg style={{ position:"absolute", inset:0, width:"100%", height:"100%" }} viewBox="0 0 300 70" preserveAspectRatio="none">
                  <path d="M20,55 Q75,55 100,35 Q150,10 180,35 Q220,60 260,25" fill="none" stroke={`${C.border}`} strokeWidth="4" strokeDasharray="6 5"/>
                  <path d="M20,55 Q75,55 100,35 Q150,10 180,35 Q220,60 260,25" fill="none" stroke={C.primary} strokeWidth="3" strokeDasharray="600" style={{ animation:"pathDash 2s ease forwards", animationDelay:"0.3s" }} strokeOpacity=".45"/>
                </svg>
                {[0,1,2,3].map(i => {
                  const xs = ["7%","35%","62%","87%"];
                  const ys = ["62%","28%","42%","12%"];
                  return (
                    <div key={i} style={{ position:"absolute", left:xs[i], top:ys[i], transform:"translate(-50%,-50%)" }}>
                      <div style={{ width:32, height:32, borderRadius:"50%", background:`linear-gradient(135deg,${C.primary},${C.quat})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:900, color:"#fff", boxShadow:"0 6px 16px rgba(163,26,97,.35)", border:"2px solid #fff" }}>{i+1}</div>
                    </div>
                  );
                })}
                <div style={{ position:"absolute", right:"2%", top:"0%", fontSize:24, animation:"float 2.5s ease-in-out infinite" }}>💰</div>
                <div style={{ position:"absolute", left:"3%", top:"50%", fontSize:22, animation:"bounce 2s ease-in-out infinite" }}>🧑‍💼</div>
              </div>
        ...
        {gameScreen === "play" && sc && (
          <div style={{ paddingBottom:32 }}>
            ...
            <div style={{ background:`linear-gradient(160deg, ${C.secondary} 0%, #21135A 100%)`, padding:"18px 18px 0", position:"relative", overflow:"hidden" }}>
              {/* Starfield */}
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:10, marginBottom:14, position:"relative", zIndex:2 }}>
                <button onClick={() => { resetGame(); setScreen("home"); }} style={{ background:"rgba(255,255,255,.12)", border:"1px solid rgba(255,255,255,.2)", borderRadius:999, padding:"8px 12px", color:"#fff", fontSize:12, cursor:"pointer", fontFamily:"inherit", fontWeight:800 }}>← Salir</button>
                <button onClick={() => setSoundOn(v => !v)} style={{ background:"rgba(255,255,255,.12)", border:"1px solid rgba(255,255,255,.2)", borderRadius:999, padding:"8px 12px", color:"#fff", fontSize:12, cursor:"pointer", fontFamily:"inherit", fontWeight:800 }}>
                  {soundOn ? "🔊" : "🔈"} Sonido
                </button>
              </div>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
                ...
                <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                  ...
                  <div style={{ padding:"4px 12px", borderRadius:20, background: gameScore >= 0 ? "rgba(11,107,64,.6)" : "rgba(185,28,28,.5)", border:`1px solid ${gameScore>=0?"#22c55e":"#ef4444"}`, fontSize:12, fontWeight:800, color:"#fff" }}>
              ...
              <div style={{ position:"relative", height:88 }}>
                <svg style={{ position:"absolute", inset:0, width:"100%", height:"100%" }} viewBox="0 0 340 88" preserveAspectRatio="none">
                  <path d="M24,72 Q85,72 110,46 Q160,12 200,42 Q240,72 290,22" fill="none" stroke="rgba(255,255,255,.12)" strokeWidth="4"/>
                  <path d="M24,72 Q85,72 110,46 Q160,12 200,42 Q240,72 290,22" fill="none" stroke="rgba(255,255,255,.4)" strokeWidth="2" strokeDasharray="8 5"/>
                </svg>
                  ...
                  return (
                    <div key={i} style={{ position:"absolute", left:wp.left, top:wp.top, transform:"translate(-50%,-50%)" }}>
                      <div style={{
                        ...
                        borderRadius:"50%",
                        background: done ? `linear-gradient(135deg,${C.success},#22c55e)` : current ? `linear-gradient(135deg,${C.quat},${C.primary})` : "rgba(255,255,255,.15)",
                        border: current ? "3px solid #fff" : done ? "2px solid #22c55e" : "2px solid rgba(255,255,255,.3)",
                        display:"flex", alignItems:"center", justifyContent:"center",
                        fontSize: current ? 14 : 12,
                        fontWeight:900, color:"#fff",
                        boxShadow: current ? "0 0 0 4px rgba(210,0,110,.4)" : "none",
                ...
                {/* Treasure */}
                <div style={{ position:"absolute", left:treasurePos.left, top:treasurePos.top, transform:"translate(-50%,-50%)", fontSize:20, animation:"float 2s ease-in-out infinite", filter: gameScreen==="result" ? "none" : "grayscale(1) opacity(.5)" }}>💰</div>
            ...
            <div style={{ padding:"16px 18px 0", animation:"slidein .35s ease" }} key={gameIndex}>
              ...
              {sc.options.map((opt, oi) => {
                ...
                if (gameChoice !== null) {
                  if (oi === gameChoice) {
                    ...
                    else if (opt.points === 0)  { bg = C.warnBg;    border = `2px solid #D97706`;      color = "#92400E"; icon = "😐"; }
              ...
              {gameChoice !== null && (() => {
                ...
                const fbBorder = isGood ? C.success : isNeutral ? "#D97706" : C.error;
                const fbColor = isGood ? C.success : isNeutral ? "#92400E" : C.error;
        ...
        {gameScreen === "result" && (() => {
          ...
          return (
            <div style={{ paddingBottom:32 }}>
              <div style={{ background:`linear-gradient(160deg, ${C.secondary} 0%, #21135A 60%, ${C.primary} 100%)`, padding:"28px 20px 36px", textAlign:"center", position:"relative", overflow:"hidden" }}>
                {["12%","30%","50%","70%","88%","20%","60%"].map((pos,idx) => (
                  <div key={idx} style={{ position:"absolute", left:pos, top:`${5+idx*13}%`, fontSize: resTier==="gold"?14:10, animation:`sparkle ${1+idx*.25}s ease-in-out infinite`, animationDelay:`${idx*.2}s`, color: resTier==="gold"?"#FDE68A":"rgba(255,255,255,.4)" }}>
                    {resTier==="gold" ? "✦" : "·"}
                  </div>
                ))}
                <div style={{ position:"relative", height:80, marginBottom:16 }}>
                  <svg style={{ position:"absolute", inset:0, width:"100%", height:"100%" }} viewBox="0 0 340 80">
                    <path d="M24,68 Q85,68 110,42 Q160,8 200,38 Q240,68 290,18" fill="none" stroke="rgba(255,255,255,.2)" strokeWidth="3"/>
                  </svg>
                  {waypoints.map((wp, wi) => (
                    <div key={wi} style={{ position:"absolute", left:wp.left, top:wp.top, transform:"translate(-50%,-50%)" }}>
                      <div style={{ width:28, height:28, borderRadius:"50%", background:`linear-gradient(135deg,${C.success},#22c55e)`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, color:"#fff", fontWeight:900, border:"2px solid rgba(255,255,255,.5)" }}>✓</div>
                    </div>
                  ))}
                  <div style={{ position:"absolute", left:treasurePos.left, top:treasurePos.top, transform:"translate(-50%,-140%)", fontSize:22, animation:"bounce 1.5s ease-in-out infinite" }}>🧑‍💼</div>
                  <div style={{ position:"absolute", left:treasurePos.left, top:treasurePos.top, transform:"translate(-50%,-50%)", fontSize:28, animation:"treasureOpen 1s ease .3s both" }}>💰</div>
                </div>
                ...
                <div style={{ fontSize:40, fontWeight:900, color: resTier==="gold"?"#FDE68A":"#fff", marginBottom:4 }}>
                ...
                {resTier === "gold" && (
                  <div style={{ display:"inline-block", padding:"4px 14px", borderRadius:20, background:"rgba(255,255,255,.2)", fontSize:13, fontWeight:800, color:"#fff" }}>
                    {lastGameAward > 0 ? "+40 XP desbloqueados · tesoro abierto ✨" : "Tesoro abierto · XP ya acreditado antes ✨"}
                  </div>
                )}
              </div>
              <div style={{ padding:"20px 18px 0" }}>
                <div style={{ background:C.bg, border:`1px solid ${C.border}`, borderRadius:18, padding:"18px 16px", marginBottom:16, boxShadow:"0 2px 8px rgba(22,15,65,.06)" }}>
                  ...
                  {gameHistory.map((h, hi) => {
                    ...
                    const rowBorder = isGood ? `${C.success}40` : isNeutral ? "#D9770640" : `${C.error}40`;
                    const rowColor = isGood ? C.success : isNeutral ? "#92400E" : C.error;
                ...
                <button onClick={() => setScreen("quizmode")} style={{ width:"100%", padding:15, borderRadius:14, background:C.bg, border:`2px solid ${C.quat}`, color:C.quat, fontSize:14, fontWeight:800, cursor:"pointer", fontFamily:"inherit", marginBottom:10 }}>
                  Ir al quiz 🧠
                </button>
