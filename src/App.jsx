...
function getGameScenarios() {
  return shuffle(GAME_SCENARIO_BANK).slice(0, 10);
}
...
export default function FinanzasTaller() {
  ...
  const startGame = () => { const sc = getGameScenarios(); setActiveScenarios(sc); setGameIndex(0); setGameScore(0); setGameChoice(null); setGameHistory([]); setGameScreen("play"); setScreen("game"); };
  const handleGameChoice = (oi) => {
    if (gameChoice !== null) return;
    const opt = activeScenarios[gameIndex].options[oi];
    setGameChoice(oi); setGameScore(s => s + opt.points); setGameHistory(h => [...h, { scenario: gameIndex, choice: oi, points: opt.points }]);
  };
  const nextScenario = () => {
    if (gameIndex + 1 >= activeScenarios.length) { if (gameScore >= 70) setXp(x => x + 80); setCompletedModules(m => ({ ...m, game: true })); setGameScreen("result"); }
  ...
  if (screen === "game") {
    ...
    // Result tier — max posible: 10 escenarios × 10 pts = 100 pts
    const tier = gameScore >= 70 ? "gold" : gameScore >= 30 ? "silver" : "bronze";
    const tierData = {
      gold:   { label: "¡Experto Financiero!", color: "#D97706", bg: "linear-gradient(135deg,#FDE68A,#F59E0B)", icon: "🏆", xpLabel: "+80 XP" },
      ...
      <div style={appSt}>
        ...
        {gameScreen === "intro" && (
          <div style={{ padding: "0 0 32px" }}>
            {/* Hero header */}
            <div style={{ background: `linear-gradient(160deg, ${C.secondary} 0%, #2D1B69 60%, #4A1259 100%)`, padding: "28px 20px 32px", position: "relative", overflow: "hidden" }}>
              {/* Stars */}
              {["18%","45%","72%","85%","30%"].map((l,i) => (
                <div key={i} style={{ position:"absolute", left:l, top:`${10+i*12}%`, fontSize:10, opacity:.5, animation:`pulse ${1.5+i*.4}s ease-in-out infinite`, animationDelay:`${i*.3}s`, color:"#fff" }}>✦</div>
              ))}
              <button onClick={() => setScreen("home")} style={{ background:"rgba(255,255,255,.12)", border:"1px solid rgba(255,255,255,.2)", borderRadius:8, padding:"6px 12px", color:"#fff", fontSize:12, cursor:"pointer", fontFamily:"inherit", marginBottom:16 }}>← Volver</button>
              <div style={{ fontSize:48, marginBottom:8, animation:"float 3s ease-in-out infinite" }}>🧭</div>
              <div style={{ fontSize:22, fontWeight:900, color:"#fff", marginBottom:6 }}>Aventura Financiera</div>
              <div style={{ fontSize:13, color:"rgba(255,255,255,.75)", lineHeight:1.6 }}>
                Tu personaje recorre un camino de 4 paradas. En cada una, toma una decisión financiera. Al final te espera un tesoro.
              </div>
            </div>

            {/* Mini path preview */}
            <div style={{ margin:"20px 18px 0", background: C.bg, border:`1px solid ${C.border}`, borderRadius:20, padding:"22px 18px", boxShadow:"0 2px 12px rgba(22,15,65,0.07)" }}>
              <div style={{ fontSize:12, fontWeight:800, color:C.textLight, letterSpacing:1.5, textTransform:"uppercase", marginBottom:16 }}>El camino te espera</div>
              {/* Inline path diagram */}
              <div style={{ position:"relative", height:70, marginBottom:18 }}>
                {/* Road line */}
                <svg style={{ position:"absolute", inset:0, width:"100%", height:"100%" }} viewBox="0 0 300 70" preserveAspectRatio="none">
                  <path d="M20,55 Q75,55 100,35 Q150,10 180,35 Q220,60 260,25" fill="none" stroke={`${C.border}`} strokeWidth="3" strokeDasharray="6 4"/>
                  <path d="M20,55 Q75,55 100,35 Q150,10 180,35 Q220,60 260,25" fill="none" stroke={C.primary} strokeWidth="2" strokeDasharray="600" style={{ animation:"pathDash 2s ease forwards", animationDelay:"0.3s" }} strokeOpacity=".4"/>
                </svg>
                {[0,1,2,3].map(i => {
                  const xs = ["7%","35%","62%","87%"];
                  ...
                  return (
                    <div key={i} style={{ position:"absolute", left:xs[i], top:ys[i], transform:"translate(-50%,-50%)" }}>
                      <div style={{ width:28, height:28, borderRadius:"50%", background:`linear-gradient(135deg,${C.primary},${C.quat})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:900, color:"#fff", boxShadow:"0 2px 8px rgba(163,26,97,.35)", border:"2px solid #fff" }}>{i+1}</div>
                    </div>
                  );
                })}
                {/* Treasure at end */}
                <div style={{ position:"absolute", right:"2%", top:"0%", fontSize:22, animation:"float 2.5s ease-in-out infinite" }}>💰</div>
                {/* Explorer */}
                <div style={{ position:"absolute", left:"3%", top:"50%", fontSize:20, animation:"bounce 2s ease-in-out infinite" }}>🧑‍💼</div>
              </div>

              {[
                { icon:"🏦", label:"Presupuesto y Ahorro" },
                { icon:"🧠", label:"Control de Impulsos" },
                { icon:"🛡️", label:"Ciberseguridad y Fraudes" },
                { icon:"📈", label:"Inversión Inteligente" },
              ].map((t,i) => (
                <div key={i} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 12px", borderRadius:10, background:C.bgSoft, marginBottom:6 }}>
                  <span style={{ fontSize:16 }}>{t.icon}</span>
                  <span style={{ fontSize:13, color:C.textMid, fontWeight:600 }}>Parada {i+1}</span>
                  <span style={{ fontSize:12, color:C.textSoft, marginLeft:"auto" }}>{t.label}</span>
                </div>
              ))}
            </div>

            <div style={{ padding:"18px 18px 0" }}>
              <button onClick={startGame} style={{ width:"100%", padding:18, borderRadius:16, background:`linear-gradient(135deg, ${C.quat}, ${C.primary})`, border:"none", color:"#fff", fontSize:16, fontWeight:900, cursor:"pointer", fontFamily:"inherit", animation:"glow 2s ease-in-out infinite", letterSpacing:".3px" }}>
                ¡Comenzar aventura! 🗺️
              </button>
            </div>
          </div>
              ...
              {sc.options.map((opt, oi) => {
                ...
                return (
                  <button key={oi} onClick={() => handleGameChoice(oi)} style={{
                    ...
                    transform,
                    boxShadow: gameChoice === null ? "0 1px 4px rgba(22,15,65,.05)" : "none",
                  }}>
        ...
        {gameScreen === "result" && (
          <div style={{ paddingBottom:32 }}>
            ...
            <div style={{ background:`linear-gradient(160deg, ${C.secondary} 0%, #2D1B69 60%, #4A1259 100%)`, padding:"28px 20px 36px", textAlign:"center", position:"relative", overflow:"hidden" }}>
              {/* Sparkles */}
              {["12%","30%","50%","70%","88%","20%","60%"].map((l,i) => (
                <div key={i} style={{ position:"absolute", left:l, top:`${5+i*13}%`, fontSize:tier==="gold"?14:10, animation:`sparkle ${1+i*.25}s ease-in-out infinite`, animationDelay:`${i*.2}s`, color: tier==="gold"?"#FDE68A":"rgba(255,255,255,.4)" }}>
                  {tier==="gold" ? "✦" : "·"}
                </div>
              ))}
              ...
              <div style={{ fontSize:52, marginBottom:8, animation:"treasureOpen 1s ease .5s both", display:"inline-block" }}>{tierData.icon}</div>
              <div style={{ fontSize:22, fontWeight:900, color:"#fff", marginBottom:6 }}>{tierData.label}</div>
              <div style={{ fontSize:40, fontWeight:900, color: tier==="gold"?"#FDE68A": tier==="silver"?"#D1D5DB":"#FDE68A", marginBottom: 4 }}>
                {gameScore >= 0 ? "+" : ""}{gameScore} pts
              </div>
              {tierData.xpLabel && (
                <div style={{ display:"inline-block", padding:"4px 14px", borderRadius:20, background:"rgba(255,255,255,.2)", fontSize:13, fontWeight:800, color:"#fff" }}>
                  {tierData.xpLabel} desbloqueados ✨
                </div>
              )}
            </div>
            ...
            <div style={{ padding:"20px 18px 0" }}>
              ...
              <div style={{ background:C.bg, border:`1px solid ${C.border}`, borderRadius:18, padding:"18px 16px", marginBottom:16, boxShadow:"0 2px 8px rgba(22,15,65,.06)" }}>
                ...
                {gameHistory.map((h, i) => {
                  ...
                  return (
                    <div key={i} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 12px", borderRadius:12, background:rowBg, border:`1px solid ${rowBorder}`, marginBottom:8 }}>
                      <div style={{ fontSize:16 }}>{rowIcon}</div>
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:12, color:C.textSoft }}>Parada {i+1}</div>
                        <div style={{ fontSize:12, color:C.textMid, fontWeight:600, marginTop:1 }}>
                          {activeScenarios[h.scenario]?.situation.slice(0,48)}…
                        </div>
                      </div>
              ...
              <button onClick={() => { setGameScreen("intro"); }} style={{ width:"100%", padding:15, borderRadius:14, background:C.bgSoft, border:`1px solid ${C.border}`, color:C.textMid, fontSize:14, fontWeight:800, cursor:"pointer", fontFamily:"inherit" }}>
                Nueva aventura 🔄
              </button>
            </div>
          </div>
